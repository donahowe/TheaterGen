...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, return_attntion_probs=False, object_positions=None, **cross_attention_kwargs):
...
class AttnProcessor:
    ...
    def __call_fast__(
        ...
        if input_ndim == 4:
            ...
            object_positions = nn.functional.interpolate(object_positions, [height, width], scale_factor=8, mode='bilinear', align_corners=False) 
            object_positions = object_positions.repeat(channel, 1)
            object_positions = object_positions.view(batch_size, channel, height * width).transpose(1, 2)
            hidden_states+=object_positions 
