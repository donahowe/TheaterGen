...
def load_sam():
    sam_model = SamModel.from_pretrained("models/sam_vit_ckpt").to(torch_device)
    sam_processor = SamProcessor.from_pretrained("models/sam_vit_ckpt")
    sam_model_dict = dict(
...
def sam_refine_attn(input_boxes_w, obj_id, sam_input_image, token_attn_np, model_dict, height, width, H, W, use_box_input, gaussian_sigma, mask_th_for_box, n_erode_dilate_mask_for_box, mask_th_for_point, discourage_mask_below_confidence, discourage_mask_below_coarse_iou, verbose):
    ...
    H = int(height/8)
    W = int(width/8)
    token_attn_np_smooth=1
    mask_size_scale = [32, 32]
    if use_box_input:
        # box input
        print("box input sam")
        ...
        masks_512, conf_scores1 = sam_box_input(model_dict, image=sam_input_image, input_boxes=input_boxes, target_mask_shape=(height, width))
    else:
        print("point input sam")
        ...
        masks, conf_scores = sam_box_input(model_dict, image=sam_input_image, input_boxes=input_boxes_w, target_mask_shape=(H, W))
        masks_512, conf_scores1 = sam_box_input(model_dict, image=sam_input_image, input_boxes=input_boxes_w, target_mask_shape=(height, width))
        
    
    # Assuming one image, one three-masks per image (so we have indexing twice)
    three_masks = masks[0][0]
    three_masks_512 = masks_512[0][0]
    ...
    mask_selected, conf_score_selected = select_mask(three_masks, conf_scores, #coarse_ious=coarse_ious, 
                                                         ...
                                                         verbose=True)
    mask_selected_512, conf_score_selected_512 = select_mask(three_masks_512, conf_scores1,
                                                         rule="largest_over_conf", 
                                                         discourage_mask_below_confidence=discourage_mask_below_confidence, 
                                                         discourage_mask_below_coarse_iou=discourage_mask_below_coarse_iou,
                                                         verbose=True)

    return mask_selected, conf_score_selected,mask_selected_512, conf_score_selected_512 
