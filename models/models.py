...
def load_sd(key="runwayml/stable-diffusion-v1-5", use_fp16=False, load_inverse_scheduler=False, use_dpm_multistep_scheduler=False, scheduler_cls=None):
    ...
    vae = AutoencoderKL.from_pretrained(key+"vae", torch_dtype=dtype).to(torch_device)
    tokenizer = CLIPTokenizer.from_pretrained(key + "tokenizer", torch_dtype=dtype)
    text_encoder = CLIPTextModel.from_pretrained(key + "text_encoder", torch_dtype=dtype).to(torch_device)
    unet = UNet2DConditionModel.from_pretrained(key + "unet", torch_dtype=dtype).to(torch_device)


    if scheduler_cls is None: # Default setting (for compatibility)
        if use_dpm_multistep_scheduler:
            scheduler = DPMSolverMultistepScheduler.from_pretrained(key + "scheduler", torch_dtype=dtype)
        else:
            scheduler = DDIMScheduler.from_pretrained(key + "scheduler", torch_dtype=dtype)
    else:
