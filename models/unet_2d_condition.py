...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        down_block_res_samples = (sample,) #存储下采样块的结果样本
        cross_attention_probs_down = [] #存储下采样块的交叉注意力概率
        ...
        for i, downsample_block in enumerate(self.down_blocks):
            ...
            # 如果下采样块具有交叉注意力，执行下采样块的操作，并处理交叉注意力输出
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                sample, res_samples = downsample_block(hidden_states=sample, temb=emb, scale=lora_scale)#, object_positions = object_positions)

                if is_adapter and len(down_block_additional_residuals) > 0:
                    sample += down_block_additional_residuals.pop(0)

            down_block_res_samples += res_samples  # 将下采样块的结果样本添加到down_block_res_samples中
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            # 如果尚未到达最终块并且需要转发上采样大小，执行此操作
            if not is_final_block and forward_upsample_size:
                upsample_size = down_block_res_samples[-1].shape[2:]

            # 如果上采样块具有交叉注意力，执行上采样块的操作，并处理交叉注意力输出
        ...
        sample = self.conv_out(sample) # 对输出样本进行后处理，可能是卷积操作或其他处理
