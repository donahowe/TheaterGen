...
def parse_input_with_negative(text=None, no_input=False):
    ...
    for i in gen_boxes:
        obj_ids.append(i[-1])
        new_boxes.append(i[:-1])


    return obj_ids, new_boxes, bg_prompt, neg_prompt
...
def convert_spec(spec, height, width, include_counts=True, verbose=False):
    # Infer from spec
    obj_ids, prompt, gen_boxes, bg_prompt = spec['obj_ids'], spec['prompt'], spec['gen_boxes'], spec['bg_prompt']
    ...
    sorted_data = sorted(zip(gen_boxes, obj_ids), key=lambda x: x[0])
    gen_boxes = [data[0] for data in sorted_data]
    obj_ids = [data[1] for data in sorted_data]
    ...
    return objects, bg_prompt, so_prompt_phrase_word_box_list, overall_prompt, overall_phrases_words_bboxes, obj_ids
