...
class IPAttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        # Currently only process cross-attention
        if attn_process_fn is not None and cross_attn:
            ip_attention_probs_before_process = ip_attention_probs.clone()
            ip_attention_probs = attn_process_fn(ip_attention_probs, query, ip_key, ip_value, attn_key=attn_key, cross_attn=cross_attn, batch_size=batch_size, heads=attn.heads)
        else:
            ip_attention_probs_before_process = ip_attention_probs
...
class AttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        attention_probs, hidden_states = F.scaled_dot_product_attention(query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False)
...
class IPAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        attention_probs, ip_hidden_states = F.scaled_dot_product_attention(query, ip_key, ip_value, attn_mask=None, dropout_p=0.0, is_causal=False)
